...
magFilter:e.NearestFilter,format:e.RGBAFormat});h.scissorTest=!0;h.texture.generateMipmaps=!1;var g=new e.Vector2(.441,.156),k=(new e.PlaneGeometry(1,1,10,20)).removeAttribute("normal").toNonIndexed(),l=k.attributes.position.array,m=k.attributes.uv.array;k.attributes.position.count*=2;k.attributes.uv.count*=2;var p=new Float32Array(2*l.length);p.set(l);p.set(l,l.length);var q=new Float32Array(2*m.length);q.set(m);q.set(m,m.length);m=new e.Vector2;l=l.length/3;for(var f=0,t=p.length/3;f<t;f++){m.x=
